...
urine_counts = df['Urine pushing (continuous need for urination)'].value_counts().sort_index()

print(f"\n{'='*60}")
print("Kolom: Urine pushing (continuous need for urination)")
print(f"{'='*60}")

# Loop untuk menampilkan setiap nilai
total = 0
for temp, count in urine_counts.items():
    print(f"{temp}: {count}")
    total += count
