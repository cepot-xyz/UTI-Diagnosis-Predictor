...
inflammation_counts = df['Inflammation of urinary bladder'].value_counts().sort_index()

print(f"\n{'='*60}")
print("Kolom: Inflammation of urinary bladder")
print(f"{'='*60}")

# Loop untuk menampilkan setiap nilai
total = 0
for temp, count in inflammation_counts.items():
    print(f"{temp}: {count}")
    total += count
